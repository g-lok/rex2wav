...
namespace REX {
  ...
  static const unsigned short kREXCompatibleMajor =
      (short)(REX_API_VERSION / 1000000L);

  static REX_int32_t gREXLoadCount = 0;

#if REX_WINDOWS
  static HINSTANCE gREXDLLInstance = 0;
  static const WCHAR kWinREXDebugDLLPathName[] =
      L"REX Shared Library Debugging.dll";
  static const WCHAR kWinREXBetaDLLPathName[] =
      L"REX Shared Library Testing.dll";
  static const WCHAR kWinREXReleaseDLLPathName[] = L"REX Shared Library.dll";
#endif /* REX_WINDOWS */

#if REX_MAC
  static CFBundleRef gREXMachODLLInstance = NULL;
  static const char kMacREXDebuggingMachODLLFileName[] =
      "REX Shared Library Debugging.bundle";
  static const char kMacREXTestingMachODLLFileName[] =
      "REX Shared Library Testing.bundle";
  static const char kMacREXDeploymentMachODLLFileName[] =
  ...
  */
  ...
  static char WinGetFileVersion(LPCWSTR pathName, short *outRevision,
                                short *outVersion, short *outBuildHi,
                                short *outBuildLo) {
    DWORD lpdwHandle = 0;
    DWORD lSize;
    UINT lOldErrorMode;
  ...
  static REXError VerifyREXDLLVersion() {
    ...
    }
    {
      ...
      short buildHi;
      short buildLo;
      WinGetFileVersion(completeFileName, &revision, &version, &buildHi,
                        &buildLo);
  ...
  static REXError AppendToPathSafely(LPWSTR ioCurrentPath,
                                     LPCWSTR iPathToAppend) {
    ...
    if (currentLength + appendLength + 1 < MAX_PATH) {
      if ((currentLength != 0) && (ioCurrentPath[0] != L'\\')) {
  ...
  */
  ...
  static char MacGetMacURLRefForREXDLLByName(
      const char *iDirPathUTF8, const char dllName[], CFURLRef *outFoundURL) {
    ...
    CFStringRef pathCFString = CFStringCreateWithCString(
        kCFAllocatorDefault, iDirPathUTF8, kCFStringEncodingUTF8);
    CFStringRef nameCFString = CFStringCreateWithCString(
        kCFAllocatorDefault, dllName, kCFStringEncodingUTF8);
    CFURLRef dirURL = CFURLCreateWithFileSystemPath(
        kCFAllocatorDefault, pathCFString, kCFURLPOSIXPathStyle, TRUE);
    CFRelease(pathCFString);
    CFURLRef bundleURL = CFURLCreateWithFileSystemPathRelativeToBase(
        kCFAllocatorDefault, nameCFString, kCFURLPOSIXPathStyle, FALSE, dirURL);
  ...
  static char MacGetURLRefForREXDLL(const char *iDirPathUTF8,
                                    CFURLRef *outFoundURL) {
    REX_ASSERT(outFoundURL != NULL);

    if (MacGetMacURLRefForREXDLLByName(
            iDirPathUTF8, kMacREXDebuggingMachODLLFileName, outFoundURL)) {
      REX_ASSERT(*outFoundURL != NULL);
      return TRUE;
    }
    if (MacGetMacURLRefForREXDLLByName(
            iDirPathUTF8, kMacREXTestingMachODLLFileName, outFoundURL)) {
      REX_ASSERT(*outFoundURL != NULL);
      return TRUE;
    }
    if (MacGetMacURLRefForREXDLLByName(
            iDirPathUTF8, kMacREXDeploymentMachODLLFileName, outFoundURL)) {
      REX_ASSERT(*outFoundURL != NULL);
      return TRUE;
    }
    return FALSE;
  ...
  static void* FindREXDLLFunction(const char functionName[]) {
    ...
    if (gREXMachODLLInstance != NULL) {
      functionNameStringRef = CFStringCreateWithCString(
          kCFAllocatorDefault, functionName, kCFStringEncodingUTF8);
  ...
  static void UnloadREXDLL() {
    if (gREXMachODLLInstance != NULL) {
      /*
              JP: Work-around for nasty problem with GCC 3.3: if any exceptions
         has been thrown (and caught) inside the REX library, the application
         will crash on exit if we release the REX bundle here. Therefore, only
         CFRelease the bundle on x86 (where the REX library is built using GCC
         4).

              JE: Keeping old comment above for historical context. It explains
         why the CFRelease() below was #if for 32 bit build only. We are a long
         way from GCC 3/4 times now, and it seems we should call CFRelease()
         here. But we don't want to crash on exit.
      */
  ...
  */
  ...
  typedef REXError(REXCALL * TREXCreateProc)(
      REXHandle *, const char[], REX_int32_t, REXCreateCallback, void *);
  typedef void(REXCALL * TREXDeleteProc)(REXHandle *);
  typedef REXError(REXCALL * TREXGetInfoProc)(REXHandle, REX_int32_t,
                                              REXInfo *);
  typedef REXError(REXCALL * TREXGetInfoFromBufferProc)(
      REX_int32_t, const char[], REX_int32_t, REXInfo *);
  typedef REXError(REXCALL * TREXGetCreatorInfoProc)(REXHandle, REX_int32_t,
                                                     REXCreatorInfo *);
  typedef REXError(REXCALL * TREXGetSliceInfoProc)(REXHandle, REX_int32_t,
                                                   REX_int32_t, REXSliceInfo *);
  typedef REXError(REXCALL * TREXSetOutputSampleRateProc)(REXHandle,
                                                          REX_int32_t);
  typedef REXError(REXCALL * TREXRenderSliceProc)(REXHandle, REX_int32_t,
                                                  REX_int32_t, float *[2]);
  typedef REXError(REXCALL * TREXStartPreviewProc)(REXHandle);
  typedef REXError(REXCALL * TREXStopPreviewProc)(REXHandle);
  typedef REXError(REXCALL * TREXRenderPreviewBatchProc)(REXHandle, REX_int32_t,
                                                         float *[2]);
  ...
  static const char kREXSetOutputSampleRateProcName[] =
  ...
  REXError REXCreate(REXHandle * handle, const char buffer[], REX_int32_t size,
                     REXCreateCallback callbackFunc, void *userData) {
  ...
  REXError REXGetInfoFromBuffer(REX_int32_t bufferSize, const char buffer[],
                                REX_int32_t infoSize, REXInfo *info) {
  ...
  REXError REXGetCreatorInfo(REXHandle handle, REX_int32_t creatorInfoSize,
  ...
  REXError REXGetSliceInfo(REXHandle handle, REX_int32_t sliceIndex,
  ...
  REXError REXRenderSlice(REXHandle handle, REX_int32_t sliceIndex,
                          REX_int32_t bufferFrameLength,
                          float *outputBuffers[2]) {
    if (!IsREXLoadedByThisInstance() || gREXRenderSliceProc == NULL) {
      return kREXImplError_DLLNotLoaded;
    } else {
      return gREXRenderSliceProc(handle, sliceIndex, bufferFrameLength,
                                 outputBuffers);
    }
  }
  ...
  REXError REXRenderPreviewBatch(REXHandle handle, REX_int32_t framesToRender,
                                 float *outputBuffers[2]) {
...
REXError REXInitializeDLL_DirPath(const char *iDirPathUTF8) {
  ...
  REX_ASSERT(iDirPathUTF8 != NULL);
    ...
    if (IsREXLoadedByThisInstance()) {
      REX_ASSERT(FALSE); /* REXLoadDLL() should only be called once before it is
                            unloaded by calling REXUninitializeDLL()! */
      gREXLoadCount++;
      ...
      REXError result = LoadREXDLL(iDirPathUTF8);
        ...
        if (result == kREXError_NoError) {
          ...
          if (result != kREXError_NoError) {
            ...
            gDLLOpenProc = (TDLLOpenProc)FindREXDLLFunction_Fallback(
                kREXInitializeDLLProcName, kDLLOpenProcName);
            gDLLCloseProc = (TDLLCloseProc)FindREXDLLFunction_Fallback(
                kREXUninitializeDLLProcName, kDLLCloseProcName);
            gREXCreateProc =
                (TREXCreateProc)FindREXDLLFunction(kREXCreateProcName);
            gREXDeleteProc =
                (TREXDeleteProc)FindREXDLLFunction(kREXDeleteProcName);
            gREXGetInfoProc =
                (TREXGetInfoProc)FindREXDLLFunction(kREXGetInfoProcName);
            gREXGetInfoFromBufferProc =
                (TREXGetInfoFromBufferProc)FindREXDLLFunction(
                    kREXGetInfoFromBufferProcName);
            gREXGetCreatorInfoProc = (TREXGetCreatorInfoProc)FindREXDLLFunction(
                kREXGetCreatorInfoProcName);
            gREXGetSliceInfoProc = (TREXGetSliceInfoProc)FindREXDLLFunction(
                kREXGetSliceInfoProcName);
            gREXSetOutputSampleRateProc =
                (TREXSetOutputSampleRateProc)FindREXDLLFunction(
                    kREXSetOutputSampleRateProcName);
            gREXRenderSliceProc = (TREXRenderSliceProc)FindREXDLLFunction(
                kREXRenderSliceProcName);
            gREXStartPreviewProc = (TREXStartPreviewProc)FindREXDLLFunction(
                kREXStartPreviewProcName);
            gREXStopPreviewProc = (TREXStopPreviewProc)FindREXDLLFunction(
                kREXStopPreviewProcName);
            gREXRenderPreviewBatchProc =
                (TREXRenderPreviewBatchProc)FindREXDLLFunction(
                    kREXRenderPreviewBatchProcName);
            gREXSetPreviewTempoProc =
                (TREXSetPreviewTempoProc)FindREXDLLFunction(
                    kREXSetPreviewTempoProcName);

            if ((gDLLOpenProc == NULL) || (gDLLCloseProc == NULL) ||
                (gREXCreateProc == NULL) || (gREXDeleteProc == NULL) ||
                (gREXGetInfoProc == NULL) ||
                (gREXGetInfoFromBufferProc == NULL) ||
                (gREXGetCreatorInfoProc == NULL) ||
                (gREXGetSliceInfoProc == NULL) ||
  ...
  void REXUninitializeDLL(void) {
     ...
     */
    /* REXUnloadDLL() should not be called unless a call to REXLoadDLL() has
     * succeeded! */
