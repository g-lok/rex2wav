...
#if ((!defined(REX_MAC)) || (!defined(REX_WINDOWS))) ||                        \
    ((!(REX_MAC)) && (!(REX_WINDOWS))) || ((REX_MAC) && (REX_WINDOWS))
...
/*
        Leave REX_DLL_LOADER == 1 and use REX.c to load the REX DLL / dylib
   bundle dynamically from a specified directory path. This is the default and
   recommended usage on Windows, especially from a plugin host or plugin module.
        Don't hard code the location - calculate it relative to some other known
   file, or derive the location from your installation somehow. The test app
   example shows how to calculate it relative to the location of the running
   executable module (.exe, .dll)

        You can set REX_DLL_LOADER = 0 globally to turn off the dependency on
   REX.c.

        On Mac, prefer REX_DLL_LOADER == 0, and just add a dependency on the REX
   .framework, and embed it in your app or plugin bundle. Don't forget to embed
   the framework - a plugin must never rely on / try and load REX from the
   plugin host or vice versa.

        If you want to use REX_DLL_LOADER == 1 on Mac (to load the dylib .bundle
   file manually), please make sure you can calculate the path you try to load
   it from in a general and reliable way.

        On Windows, if REX_DLL_LOADER == 0 you must use your own loader code
   instead of REX.c.

        It's also possible to link the included default generated link library
   loader .lib or framework .bundle loader (as recommended for REX
   SDK 1.8-1.9.1), however this is not supported in a shipping product,
   especially not for a plugin host or plugin, because it can break the library
   for itself, any other plugins with the same issue and the host.
*/
#define REX_ABI
...
typedef struct {
  ...
  REX_int32_t fTempo;      /* Tempo set when exported from ReCycle, 123.456 BPM
                              stored as 123456L etc. */
  REX_int32_t
      fOriginalTempo; /* Original tempo of loop, as calculated by ReCycle from
                         the locator positions and bars/beats/sign settings. */
...
typedef struct {
  ...
  REX_int32_t fSampleLength; /* Length of rendered slice, at its original sample
                                rate. */
} REXSliceInfo;
...
typedef REXCallbackResult(REXCALL *REXCreateCallback)(
    REX_int32_t percentFinished, void *userData);

#if REX_DLL_LOADER
// iDirPath is the absolute path to the directory containing REX DLL /
// framework-bundle
#if REX_WINDOWS
...
REX_ABI REXError REXCreate(REXHandle *handle, const char buffer[],
                           REX_int32_t size, REXCreateCallback callbackFunc,
                           void *userData);

REX_ABI void REXDelete(REXHandle *handle);

REX_ABI REXError REXGetInfo(REXHandle handle, REX_int32_t infoSize,
                            REXInfo *info);

REX_ABI REXError REXGetInfoFromBuffer(REX_int32_t bufferSize,
                                      const char buffer[], REX_int32_t infoSize,
                                      REXInfo *info);
...
REX_ABI REXError REXGetSliceInfo(REXHandle handle, REX_int32_t sliceIndex,
...
REX_ABI REXError REXRenderSlice(REXHandle handle, REX_int32_t sliceIndex,
...
REX_ABI REXError REXSetPreviewTempo(REXHandle handle, REX_int32_t tempo);
